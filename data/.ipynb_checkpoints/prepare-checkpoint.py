...
def main(config, TOK):
    ...
    if dataset:
        ...
        with open(os.path.join(config.out_dir, 'train.json'), 'w', encoding='utf-8') as fw:
            fw.write('\n'.join([json.dumps(ex, ensure_ascii=True) for ex in train_dataset]))
    ...
    if dataset:
        ...
        with open(os.path.join(config.out_dir, 'valid.json'), 'w', encoding='utf-8') as fw:
            fw.write('\n'.join([json.dumps(ex, ensure_ascii=True) for ex in valid_dataset]))
    ...
    if dataset:
        ...
        with open(os.path.join(config.out_dir, 'test.json'), 'w', encoding='utf-8') as fw:
            fw.write('\n'.join([json.dumps(ex, ensure_ascii=True) for ex in test_dataset]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-tokenizer', default='BertTokenizer',
                        choices=['BertTokenizer', 'SpacyTokenizer', 'WhiteSpace', 'DoNothingTokenizer'])
